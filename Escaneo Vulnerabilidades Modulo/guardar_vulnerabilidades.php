...
if (!is_array($data) || !isset($data['vulnerabilidades']) || !is_array($data['vulnerabilidades'])) {
    ...
    echo json_encode(['ok' => false, 'error' => 'JSON invalido o falta vulnerabilidades'], JSON_UNESCAPED_UNICODE);
    exit;
}
...
try {
    ...
    $sql = "
        INSERT INTO vulns_resultados (
            agent_id, agent_name, cve, severidad, cvss, paquete, version_paquete,
            descripcion, referencia, remediacion, estado, actualizado_en
        )
        VALUES (
            :agent_id, :agent_name, :cve, :severidad, :cvss, :paquete, :version_paquete,
            :descripcion, :referencia, :remediacion, :estado, CURRENT_TIMESTAMP
        )
    ...
    foreach ($data['vulnerabilidades'] as $v) {
        $stmt->execute([
            ...
            ':cvss' => is_numeric($v['score'] ?? null) && ($v['score'] ?? null) >= 0 ? $v['score'] : null,
            ':paquete' => $v['paquete'] ?? '',
