...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($accion === 'aplicar') {
            ...
            if ($politicaId > 0) {
                $stmt = $pdo->prepare("
                    ...
                    VALUES (:politica_id, :agente_id, 'aplicar', 'pendiente')
                ");
                $stmt->execute([
        ...
        if ($accion === 'refrescar') {
            $stmt = $pdo->prepare("
                INSERT INTO politicas_ordenes (politica_id, agente_id, accion, estado)
                SELECT id, :agente_id, 'verificar', 'pendiente'
...
function estadoClase(string $estado): string {
    return match ($estado) {
        'completada', 'aplicado', 'verificado' => 'estado-ok',
        'error' => 'estado-error',
    ...
    <aside class="dashboard-sidebar" id="dashboardSidebar">
        ...
        <div class="menu-category">
            ...
            <div class="submenu">
                ...
                <a href="/CIS módulo/politicas_seguridad.php">Políticas de seguridad</a>
            </div>
        </div>
